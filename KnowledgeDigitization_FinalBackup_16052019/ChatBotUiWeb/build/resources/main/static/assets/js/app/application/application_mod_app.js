fordApp.controller('applicationController', ['$scope','$timeout','$translate','appliService','$route','$window','$anchorScroll',
	'$rootScope','$filter','$http',//,'blockUI'
	function($scope,$timeout,$translate, appliService,$route,$window,$anchorScroll,$rootScope,$filter,$http) {//,blockUI
    ...
    // Function to get data for all selected items
       ...
       $scope.validate =  function($selectedData) {
    	   ...
    	   angular.forEach($selectedData, function(row) {
    		   ...
    		   if(!$scope.flag) {
    			   $scope.reqErrMsg="";
    			   if(specialCharacterNumValidation(row.itmsNo)){
        			   if($scope.flag){
            			   $scope.reqErrMsg +=  ', ' ;
            		   }
        			   ...
        			   $scope.itmsNoFlag = true;
        			   $scope.flag = true; 
        		   }else if(row.itmsNo.length!=5) {
    				   if($scope.flag){
            			   $scope.reqErrMsg +=  ', ' ;
            		   }
    				   $scope.reqErrMsg +=  '{{"appliMain.itmsNo.lengthEqual.err" | translate}}';
        			   $scope.itmsNoFlag = true;
        			   $scope.flag = true;  
    			   }else{
    			   ...
    			   if(specialCharacterValidationAlpha(row.applicationAcronym)){
     				  if($scope.flag){
            			   $scope.reqErrMsg +=  ', ' ;
            		      }
     	    			 $scope.reqErrMsg+='{{"appliMain.applicationAcronym.numaric.errMsg" | translate}}';
     	    			 $scope.appliAconymFlag = true;
     	    		       $scope.flag = true;
     	    		 }
      ...
      $scope.DownloadTemplateAppliMainten =function(){
    	  ...
    	  $scope.downloadExcelAppliMaintenCntrl =function(){
    			  ...
    			  $http.post("http://35.154.237.162:9060/ChatBotService/Application/downloadExcel",$scope.rowCollection).then(function(response) {
    		          if (response) {
    		        	  var dt = new Date(); 
    		  ...
    		  $scope.excelUploadFile = function() {
    		     ...
    		     var data = new FormData();
    		      ...
    		      {
    		    	  $http.post("http://35.154.237.162:9060/ChatBotService/Application/ExcelUpload", data, config).then(function(response) {
    		              if (response) {
    		            	  var dt = new Date(); 
    		         	     $scope.createdDate= (dt.getMonth() + 1) + "/" + dt.getDate() + "/" + dt.getFullYear()+ " " +dt.getHours()+ ":" +dt.getMinutes() + ":" + dt.getSeconds() ;
	...
	fordApp.service("appliService",['$http',function($http){
		...
		this.srchAllItmsNo = function() { 
			return new Promise(function(resolve,reject){
				...
				$http.post('http://35.154.237.162:9060/ChatBotService/Application/ItmsNoOnload').then(
					function(response){
		...
		this.srchApplicationMainServ = function(data) { 
			...
			return new Promise(function(resolve,reject){
				...
				$http.post('http://35.154.237.162:9060/ChatBotService/Application/applicationSearch',data)
				.then(
					function(response){
						resolve(response.data);
					},
		...
		this.saveAppliMaintenance = function(data) { 
			return new Promise(function(resolve,reject){
				...
				$http.post('http://35.154.237.162:9060/ChatBotService/Application/saveAppliMaintenance',data).then(
					function(response){
						resolve(response.data);
					},
					function(error){
		...
		this.updateAppliMaintenance = function(data) { 
			return new Promise(function(resolve,reject){
				...
				$http.post('http://35.154.237.162:9060/ChatBotService/Application/updateAppliMaintenance',data).then(
					function(response){
						resolve(response.data);
					},
					function(error){
		...
		{
				return new Promise(function(resolve,reject){
				$http.get("http://35.154.237.162:9060/ChatBotService/Application/downloadTemplate").then(
				function(response){
		...
		this.autoCompletePN=function(projectName){
			return new Promise(
					function(resolve, reject) {				
						$http.post('http://35.154.237.162:9060/ChatBotService/Application/searchApplicationName',projectName)
